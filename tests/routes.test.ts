...
describe("catcher /hook/[...slug]", () => {
  it("captures a POST with raw body, headers and query; returns 200", async () => {
    const req = new NextRequest("http://localhost/hook/in_abc?x=1", {
      method: "POST",
      body: '{"type":"charge.succeeded"}',
      headers: { "content-type": "application/json", "stripe-signature": "t=1,v1=z" },
    });

    const res = await catcher.POST(req, ctx(["in_abc"]));
    expect(res.status).toBe(200);

    expect(captureRequest).toHaveBeenCalledTimes(1);
    expect((captureRequest as ReturnType<typeof vi.fn>).mock.calls[0][0]).toBe("in_abc");
    const data = capturedData();
    expect(data.method).toBe("POST");
    expect(data.path).toBe("/hook/in_abc");
  ...
  it("never rejects malformed bodies — stores them raw and still 200s", async () => {
    const req = new NextRequest("http://localhost/hook/in_abc", {
      method: "POST",
      body: "{bad json",
  ...
  it("captures a GET with query and no body, and keeps the subpath", async () => {
    const req = new NextRequest("http://localhost/hook/in_abc/events?token=t", {
      method: "GET",
    });

    const res = await catcher.GET(req, ctx(["in_abc", "events"]));
    expect(res.status).toBe(200);
    const data = capturedData();
    expect(data.method).toBe("GET");
    expect(data.body).toBeNull();
    expect(data.query).toEqual({ token: "t" });
    expect(data.path).toBe("/hook/in_abc/events");
  });
  ...
  it("still acknowledges (200) if storage throws", async () => {
    ...
    const req = new NextRequest("http://localhost/hook/in_abc", { method: "POST", body: "x" });
    const res = await catcher.POST(req, ctx(["in_abc"]));
    expect(res.status).toBe(200);
    errSpy.mockRestore();
  });
